					...
					.div() // All divs need tab index to be focusable, so can be hidden on blur
						...
						.div().attrClass(CSS.FV_CONTEXT_MENU)
							...
							.div()
								...
								.__()
								.button()
									.attrId(CSS.FV_ID_RENAME_BTN + model.getIDPostFix())
									.attrClass(CSS.BUTTON_SMALL + CSS.BUTTON_IMAGE + CSS.BACKGROUND_GREEN + CSS.ICON_CHECK)
									.attrType(EnumTypeButtonType.BUTTON)
								.__()
							.__()
							.div()
								.attrId(CSS.FV_ID_DEL_BTN + model.getIDPostFix())
								.attrClass(CSS.FV_CONTEXT_MENU_ITEM)
								.attrTabIndex(-1)
								.text("Delete Folder")
							.__()
							...
							.div()
								.attrClass(CSS.FV_CONTEXT_MENU_ITEM)
								.attrTabIndex(-1)
								.attrOnclick(JS.fnGetDirIcons())
								.text("Change Icon")
							.__()
							.div()
								.attrClass(CSS.FV_CONTEXT_MENU_INPUT + CSS.FV_CONTEXT_NO_HOVER)
								.div().__()
							.__()
						.__()
					.__();
